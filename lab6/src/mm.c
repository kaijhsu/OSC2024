...
void* simple_malloc(unsigned int bytes){
    if((uint64_t)__heap_ptr + bytes >= va_kernel(HEAP_MAX)){
...
char* mframe_alloc_iter(unsigned int bytes, unsigned char* mptr, char* mbegin, char* mend){
    ...
    if(ret_ptr == 0){
        debug("Already in used mbegin: %l mend: %l", mbegin, mend);
    }
    return ret_ptr;
}
...
int mframe_free_iter(char* addr, unsigned char* mptr, char* mbegin, char* mend){
    ...
    char* mid = get_mid(mbegin, mend);
    if(status == MFRAME_FREE){
        debug("this addr %l hasn't been used", addr);
        return -1;
    }
    else if(status == MFRAME_DIVIDE){
        unsigned char* left = get_left(mptr);
        ...
        debug("this addr %l is reserved", addr);
        return -1;
    }
    debug("never go here");
    return -1;
...
void mframe_dump_iter(unsigned char* mptr, char* begin, char* end, int depth, int filter){
    ...
    if(status == MFRAME_FREE){
        if(filter < 0 || status == filter)
            uart_printf("\t["STYLE("FREE", FONT_GREEN)"] shift: %d %l to %l\n", get_shift(mptr), begin, end);
        return;
    }
    else if(status == MFRAME_DIVIDE){
        if(filter < 0 || status == filter)
            uart_printf("\t["STYLE("DIVI", FONT_BLUE)"] shift: %d %l to %l\n", get_shift(mptr), begin, end);
        ...
        if(filter < 0 || status == filter)
            uart_printf("\t["STYLE("USED", FONT_RED)"] shift: %d %l to %l\n", get_shift(mptr), begin, end);
        return;
    }
    else if(status == MFRAME_RESERVE){
        if(filter < 0 || status == filter)
            uart_printf("\t["STYLE("RESV", FONT_RED)"] shift: %d %l to %l\n", get_shift(mptr), begin, end);
        return;
    }
    debug("never go here");
}
...
uint64_t mbegin_get(){
    return (uint64_t)mem_begin;
}

uint64_t mend_get(){
    return (uint64_t)mem_end;
}
...
int mframe_get_mem_section(char* nodename, char* propname, char* propvalue, unsigned int proplen){
    if(0 == m_strcmp(nodename, "memory@0") && 0 == m_strcmp(propname, "reg")){
        ...
        debug("mem_begin end %l %l", mem_begin, mem_end);
        return 0;
    }
    return -1;
}
...
int mframe_init(){
    ...
    mbegin = mem_begin;
    mend   = mbegin + (1<<shift);
    debug("mframe init section %l %l", mbegin, mend);
    
    // reserve for unexisted memory (truncate)
    debug("reserve for unexisted memory %l %l", mem_end, mend);
    mframe_reserve((char*)mem_end, mend);

    // compute mframe size
    unsigned long mframe_size = (size / PAGE_SIZE) << 1;
    char* mframe_end = (char*)mframe + mframe_size - va_offset;
    ...
    debug("reserve for spin table, heap, stack, kernel, mframe %l %l", 0, mframe_end);
    ...
    if(0 == cpio_get_start_addr(&cpio.next)){
        ...
        cpio_begin -= va_offset;
        cpio_end   -= va_offset;
        debug("reserve for iniramfs %l %l", cpio_begin, cpio_end);
    ...
    if(0 == fdt_get_begin_end(&dt_begin, &dt_end)){
        debug("reserve for device tree %l %l", dt_begin, dt_end);
