...
int mailbox_call(unsigned char channel, volatile unsigned int mailbox[]){
    ...
    uint32_t mes_addr = (unsigned long)mailbox & 0x00000000FFFFFFFF;
    uint32_t write_val = (mes_addr & ~0xF) | (channel & 0xF);
    unsigned long align_check = (unsigned long)mailbox;
    if(align_check & 0xF)
...
void print_board_info() {
    ...
    uart_printf("board serial: %l\n", board_serial);
    uart_printf("board revision: %x\n", board_revision);
    uart_printf("arm base_addr: %x, mem_size: 0x%x\n", base_addr, mem_size);
