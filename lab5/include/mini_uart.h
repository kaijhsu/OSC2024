...
#define kprintf(fmt, ...) \
    uart_printf("["STYLE("KERNEL", FONT_BLUE)"] %s:%d, "STYLE("%s", FONT_BLUE)": "fmt, __FILE__, __LINE__, __func__, ##__VA_ARGS__)

#define throw(errno, fmt, ...) ({ \
    uart_printf("["STYLE("ERROR", FONT_RED)"] %s:%d, "STYLE("%s", FONT_CYAN)": "fmt"\n", __FILE__, __LINE__, __func__, ##__VA_ARGS__); \
    return errno; })

void uart_init ( void );
char uart_recv ( void );
char uart_last_input( void );
